...
namespace Montaniarzii.WebApp.Code
{
        ...
        public override void OnException(ExceptionContext context)
        {
            ...
            switch (context.Exception)
            {
                case NotFoundErrorException notFound:
                    ...
                    break;
                case BadRequestErrorException badRequest:
                    var hasRequestedWithHeaderForBadRequest = context.HttpContext.Request.Headers.TryGetValue("X-Requested-With", out var requestedWithHeaderValueForBadRequest);

                    if (hasRequestedWithHeaderForBadRequest && requestedWithHeaderValueForBadRequest == "XMLHttpRequest")
                    {
                ...
                case ValidationErrorException validationError:
                    ...
                    if ((hasRequestedWithHeader && requestedWithHeaderValue == "XMLHttpRequest") || descriptor.ControllerName == "Photo")
                    {
                        //context.Result = new StatusCodeResult(StatusCodes.Status412PreconditionFailed);
